...
def run_bayesian_predictor(db_path):
    ...
    for (season, week), week_games in games.groupby(["season", "week"]):
        ...
        for _, game in week_games.iterrows():
            ...
            # Build prefixed feature names for Bayesian scoring
            home_feats = {f'home_{k}': home_stats.get(k) for k in feature_keys}
            away_feats = {f'away_{k}': away_stats.get(k) for k in feature_keys}

            home_feats['home_field'] = 1.0
            away_feats['home_field'] = 0.0

            score_home = score_team(home_feats, posteriors)
            score_away = score_team(away_feats, posteriors)
            ...
            if is_regular:
                ...
                confidence = min(score_diff, 1.0)  # cap at 1.0 to avoid overconfidence

                if actual_winner == 'home':
                    update_posteriors(posteriors, home_feats, away_feats, confidence=confidence)
                else:
                    update_posteriors(posteriors, away_feats, home_feats, confidence=confidence)
