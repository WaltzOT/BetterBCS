...
priors = {
    'matchup_away_rolling_total_yards_for': (6, 2),
    'matchup_away_rolling_total_yards_against': (6, 2),
    'matchup_home_rolling_total_yards_for': (6, 2),
    'delta_rolling_total_yards_for': (6, 2),
    'delta_rolling_total_yards_against': (2, 6),
    'matchup_home_rolling_total_yards_against': (2, 6),
    'delta_rolling_elo': (6, 2),
    'matchup_home_rolling_elo': (6, 2),
    'matchup_away_rolling_elo': (2, 6),
    'home_field': (6, 4),
    'matchup_away_rolling_points_allowed': (2, 4),
...
def score_team(features, posteriors):
    ...
    for f, value in features.items():
        ...
        scale = 1 / (1 + 0.01 * (a + b))  # optional: cap or scale weight volatility
        score += weight * scale * value
...
def update_posteriors(posteriors, winning_features, losing_features, confidence=1.0):
    ...
    for f in posteriors:
        a, b = posteriors[f]
        posteriors[f] = (a * DECAY_RATE, b * DECAY_RATE)
